    ...
    favs.forEach((favs) => {
      data2 += `<tr> <td><img src="${favs.crestUrl.replace(
        ...
        "https://"
			...
			<td><a class="simpan" href="#team?id=${favs.id}">${favs.name}</a> </td> </tr> `;
    });
    data += `<div class="card bulat warna-fav"> <table border="1"> 
        ${data2 === "" ? "Tidak Ada Tim Favorit" : data2} 
			 
			</table> </div>`;

    document.getElementById("favorit").innerHTML = data;
    document.querySelectorAll(".simpan").forEach(function (lnk) {
      lnk.addEventListener("click", function (event) {
        // mengambil nilai id lalu dimasukkan ke variabel urlTeam Param
        urlTeamParam = event.target.getAttribute("href").substr(9);
        // Muat konten halaman yang dipanggil
